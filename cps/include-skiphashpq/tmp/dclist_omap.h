...
//! -  There should be some kind of "diffractor" within each bucket (basically
//!    just a fixed array of queues instead of a single queue), to randomly steer
//!    threads to one of a plurality of queues when performing an insertion or
//!    removal.  This should reduce contention within a hot bucket.  Note that
//!    this might belong in the dclist or in the chunk_queue itself.  Not sure.
...
public:
  ...
  dclist_omap(OPTSTM *me, config_t *cfg)
    ...
    typename OPTSTM::RW rw(me);
    head->next.set_cap(rw, head, tail);
    tail->prev.set_cap(rw, tail, head);
    //if (num_queues != 32) std::cout << "[dclist_omap] num_queues is " << num_queues << "\n";
    if (!rw.OP()->try_end_rw())
      std::terminate();
  }
...
private:
  ...
  ///
  /// @tparam RWTX The type of the active transaction (RO or RW)
  ///
  /// @param tx  The active transaction
  /// @param key The key for which we are doing a predecessor query.
  ///
  /// @return The node that was found
  std::optional<node_t *> get_leq(RWTX &tx, const K key) {
    ...
    while (true) {
      ...
      if (nkey == key) {
        //! if (store_prev) prev = curr; // need to re-read next when in RW mode
        return next;
      }
...
public:
  ...
  ///         on abort
  //! std::optional<std::pair<C *, std::optional<RWTX>>> get_extract(RWTX &tx, const K &key) {
  std::optional<C *> get_extract(RWTX &tx, const K &key) {
    ...
    // //! BEGIN RW
    // RWTX rw(me);
    // Re-read n using prev
    // auto n2_o = prev->next.get(tx, prev);
    // if (!n2_o) return {};
    // auto n2 = n2_o.value();
    // if (n != n2) {
  ...
  std::optional<C *> get_ins(RWTX &tx, const K &key) {
    // get_leq will use a read-only transaction to find the largest node with
    // a key <= `key`.
    auto n_o = get_leq(tx, key);
    if (!n_o)
