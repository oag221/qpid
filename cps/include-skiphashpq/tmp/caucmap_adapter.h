...
public:
  ...
  ///         on abort
  //! template <class TX> std::optional<std::pair<C *, std::optional<RWTX>>> get_extract(TX &tx, const K &key) {
  ...
  // std::optional<bool> mark_empty(RWTX &rw, const K &key) {
  //   return buckets[hash(rw.OP(), key)]->mark_empty(rw, key);
  // }
