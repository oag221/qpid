...
public:
  ...
  struct q_ptr_t : ownable_t {
    ...
    q_ptr_t(RWTX &rw, uint32_t chunksize, int max_queues) : ownable_t(), num_queues(0), open_lanes(1) {
      lanes_ptr = (FIELD<Q_T*>*)malloc(max_queues * sizeof(FIELD<Q_T*>));
    }
  ...
  chunk_queue_diffractor(RWTX &rw, uint32_t chunksize, uint32_t n_queues) :
        MAX_QUEUES(n_queues),
        lanes_wrapper(rw, chunksize, n_queues) {
    ...
    for (int i = 0; i < n_queues; i++) {
      ...
      lanes_wrapper.lanes_ptr[i].set_mine(rw, &lanes_wrapper, &(queues[i]));
    }
  }

  ~chunk_queue_diffractor() {
    free(queues); // todo: is this correct cleanup ??
  }
  ...
  template <typename TX> std::optional<uint64_t> random_queue_idx(TX &tx) {
    ...
    auto cur_num_queues_o = lanes_wrapper.num_queues.get(tx, &lanes_wrapper);
    if (!cur_num_queues_o) return {}; // ABORT! (also if num = 0)
    if (!cur_num_queues_o.value()) {
      std::cout << "chunk_queue_diffractor: cur_num_queues is 0!\n";
  ...
  bool enqueue(RWTX &rw, P &prio, E &element) {
    ...
    auto cur_num_lanes_o = lanes_wrapper.open_lanes.get(rw, &lanes_wrapper);
    ...
    auto queue_o = lanes_wrapper.lanes_ptr[rand_queue_idx].get(rw, &lanes_wrapper);
    ...
    if (enq_ret) {
      auto n_queues_o = lanes_wrapper.num_queues.get(rw, &lanes_wrapper);
      if (!n_queues_o) return {}; // ABORT!
      auto n_queues = n_queues_o.value();

      if (!lanes_wrapper.num_queues.set(rw, &lanes_wrapper, n_queues + 1)) return {}; // ABORT!
    }

    // Check if a new lane should be opened
    if (check) {
      if (!lanes_wrapper.open_lanes.set(rw, &lanes_wrapper, cur_num_lanes + 1)) return {}; // ABORT!
    }
    return true;
  }
  ...
  std::optional<bool> enqueue_batch_vec(RWTX &rw, std::vector<std::pair<P,E>> &batch, int batch_size) {
    // Get current number of lanes
    auto cur_num_lanes_o = lanes_wrapper.open_lanes.get(rw, &lanes_wrapper);
    if (!cur_num_lanes_o) return {}; // ABORT!
    auto cur_num_lanes = cur_num_lanes_o.value();
  ...
  /// Enqueue a batch (of type q_node_t) to a randomly selected, active chunk queue
  ///
  /// @param rw The current active transaction
  /// @param batch The batch (chunk) to insert (either directly, or via copying elements depending on the current state of the selected chunk queue)
  ///
  /// @return True on success, or NONE on abort
  std::optional<bool> enqueue_batch_chunk(RWTX &rw, typename Q_T::q_node_t* &batch) {
    // Get current number of lanes
    auto cur_num_lanes_o = lanes_wrapper.open_lanes.get(rw, &lanes_wrapper);
    if (!cur_num_lanes_o) return {}; // ABORT!
    auto cur_num_lanes = cur_num_lanes_o.value();
    ...
    // Call enqueue on the queue
    auto enq_ret_o = queue->enqueue_batch_chunk(rw, batch, check);
    if (!enq_ret_o) return {}; // ABORT!
    auto enq_ret = enq_ret_o.value();
  ...
  std::optional<typename Q_T::q_node_t*> dequeue(RWTX &rw, bool &empty_q) {
    ...
    // // Get the transactional object, rw
    // auto rw = std::move(ret_o.value().second);

    // Handle the case in which the queue is now empty
    if (empty_rand_q) {
      // NOTE: next line was read in random_queue_idx() - can use get_mine()?
      auto cur_num_queues_o = lanes_wrapper.num_queues.get(rw, &lanes_wrapper);
      if (!cur_num_queues_o) return {}; // ABORT! // todo: can it be 0 for cur_num_queues_o.value()..?
      if (!cur_num_queues_o.value()) {
        std::cout << "cur num queues is 0!~~\n";
        return {};
      }
      uint8_t cur_num_queues = cur_num_queues_o.value();
      ...
      // Read open_lanes
      auto cur_open_lanes_o = lanes_wrapper.open_lanes.get(rw, &lanes_wrapper);
      if (!cur_open_lanes_o) return {}; // ABORT!
      auto cur_open_lanes = cur_open_lanes_o.value();

      if (cur_open_lanes > 1) {
        // Decrement open_lanes
        if (!lanes_wrapper.open_lanes.set(rw, &lanes_wrapper, cur_open_lanes - 1)) return {}; // ABORT!
      }

      // Decrement num_queues
      if (!lanes_wrapper.num_queues.set(rw, &lanes_wrapper, cur_num_queues - 1)) return {}; // ABORT!
  ...
  bool swap(RWTX &rw, int swap_idx, int n_queues) {
    auto last_q_ptr_o = lanes_wrapper.lanes_ptr[n_queues - 1].get(rw, &lanes_wrapper);
    if (!last_q_ptr_o) return false; // ABORT!
    auto last_q_ptr = last_q_ptr_o.value();

    auto swap_q_ptr_o = lanes_wrapper.lanes_ptr[swap_idx].get(rw, &lanes_wrapper);
    if (!swap_q_ptr_o) return false; // ABORT!
    auto swap_q_ptr = swap_q_ptr_o.value();

    return lanes_wrapper.lanes_ptr[n_queues - 1].set(rw, &lanes_wrapper, swap_q_ptr) &&
           lanes_wrapper.lanes_ptr[swap_idx].set(rw, &lanes_wrapper, last_q_ptr);
  }

  // SINGLE threaded only
  void print_queues(RWTX &rw) {
    auto cur_num_queues = lanes_wrapper.num_queues.get(rw, &lanes_wrapper).value();

    for (int i = 0; i < cur_num_queues; i++) {
      auto cur_q = lanes_wrapper.lanes_ptr[i].get(rw, &lanes_wrapper).value();
      cur_q->dump();
    }
  }
  ...
  std::optional<bool> empty(RWTX &rw) {
    auto num_queues_o = lanes_wrapper.num_queues.get(rw, &lanes_wrapper);
  ...
  template <typename TX> std::pair<long,long> dump(TX &ro, bool print=false) {
    ...
    for (int i = 0; i < MAX_QUEUES; i++) {
      ...
      auto lanes_ptr = lanes_wrapper.lanes_ptr[i].get(ro, &(lanes_wrapper)).value();
      std::pair<long,long> ret = lanes_ptr->dump(ro, print);
      num_elems += ret.first;
      key_sum += ret.second;
