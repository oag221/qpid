...
public:
  ...
  ///
  /// @return A chunk from the queue, or NONE on abort
  std::optional<E> dequeue_strict(RWTX &rw, bool &empty_q) {
    // Get target to unstitch from
    ...
    return target->elements[deqs].get_unsafe();
  }
  ...
  std::optional<bool> enqueue(RWTX &rw, E &element, bool &check) {
    ...
    if (target == head) {
      ...
      new_target->elements[0].set_cap(rw, new_target, element);
  ...
  ///
  /// @return true if inserted to this queue for first time, false otherwise (std::nullopt for Abort)
  std::optional<bool> ins_many(RWTX &rw, std::vector<E> &batch, int batch_size, int batch_idx, q_node_t *prev_) {
    ...
    while (to_process) {
      ...
      for (i = 0; i < NUM_ELEMENTS; i++) {
        ...
        new_target->elements[i].set_cap(rw, new_target, batch[batch_idx++]);
      }
      to_process -= i;
  ...
  std::optional<bool> enqueue_batch_vec(RWTX &rw, std::vector<E> &batch, int batch_size, bool &check) { // check == true (when passed) iff cur_num_lanes < MAX_QUEUES && current queue is last "active" one
    // Get tail & tail->prev
    q_node_t* tail = &tail_;
    auto target_o = tail->prev.get(rw, tail);
    if (!target_o)
    ...
    if (batch_size <= target_room) {
      ...
      for (e_idx = enqs; e_idx < NUM_ELEMENTS; e_idx++) {
        ...
        if (batch_idx == 0) {
          if (!target->elements[e_idx].set(rw, target, batch[batch_idx++])) return {}; // ABORT!
        } else {
          target->elements[e_idx].set_mine(rw, target, batch[batch_idx++]);
        }
      }
  ...
  std::optional<bool> enqueue_batch_chunk(RWTX &rw, q_node_t* &batch, bool &check) { // check == true (when passed) iff cur_num_lanes < MAX_QUEUES && current queue is last "active" one
    ...
    if ((batch_size + target_size) <= NUM_ELEMENTS) { //! need to make sure all chunks are num_elements length for this to work (or else, account for it otherwise - store max size in chunk when creating)
      ...
      for (i = target_size; i < (target_size + batch_size); i++) {
        auto j_move = batch->elements[idx_batch++].get_unsafe();
        if (!target->elements[i].set(rw, target, j_move)) return {}; // ABORT!
      }
