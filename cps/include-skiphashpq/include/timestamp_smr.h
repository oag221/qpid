...
public:
  ...
  /// the global list
  timestamp_smr_t(global_t &_globals_) : ts(ULLONG_MAX) {
    while (true) {
      timestamp_smr_t *curr_head = _globals_.all_threads;
      next = curr_head;
      if (_globals_.all_threads.compare_exchange_strong(curr_head, this))
        break;
    }
  }
