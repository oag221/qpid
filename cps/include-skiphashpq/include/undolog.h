...
// /// undolog_t is a vector of address/value pairs that are used to log and undo
// /// writes to memory in the event that an operation must roll back.
// ///
// /// undolog_t also provides helper functions for reading/writing memory properly
// /// with respect to the C++ memory model.
// class undolog_t {
// public:
...
//     ///
//     /// Note that we want to preserve these types, so that atomic_ref-based
//     /// writes from this file are of the same granularity as program
//     /// reads/writes.
//     int type;
...
//       switch (type) {
//       case 0:
//         std::atomic_ref(*addr.u1).store(val.u1, std::memory_order_release);
//         break;
//       case 1:
//         std::atomic_ref(*addr.u2).store(val.u2, std::memory_order_release);
//         break;
//       case 2:
//         std::atomic_ref(*addr.u4).store(val.u4, std::memory_order_release);
//         break;
//       case 3:
//         std::atomic_ref(*addr.u8).store(val.u8, std::memory_order_release);
//         break;
//       case 4:
//         std::atomic_ref(*addr.f4).store(val.f4, std::memory_order_release);
//         break;
//       case 5:
//         std::atomic_ref(*addr.f8).store(val.f8, std::memory_order_release);
//         break;
//       case 6:
//         std::atomic_ref(*addr.f16).store(val.f16, std::memory_order_release);
//         break;
//       case 7:
//         *addr.p = val.p;
//         break;
...
// public:
//   /// Construct an undo log by providing a default size for the underlying
//   /// vector
//   undolog_t(const unsigned long _capacity = 64) : undolog(_capacity) {}

//   /// Undo the writes stored in this undo log
//   void undo_writes() {
...
//   /// Insert an undo_t into the undo log
...
//     // return std::atomic_ref(*addr).load(std::memory_order_acquire);
//     return *addr;
//   }

//   /// Perform a speculative write to memory, by casting `addr` to an atomic ref
//   template <typename T> static void safe_write(T *addr, T val) {
//     // std::atomic_ref(*addr).store(val, std::memory_order_release);
//     *addr = val;
//   }
...
/// undolog_atomic.h defines an undo log that also provides helper functions for
