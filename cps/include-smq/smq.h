...
public:
  ...
  bool compare(T elem1, T elem2) {
    if (ORDER) return elem2.first > elem1.first;
  ...
  Galois_SMQ::optional<std::array<T, STEAL_NUM>> trySteal(bool& raceHappened) {
    auto emptyRes = Galois_SMQ::optional<std::array<T, STEAL_NUM>>();
    auto v1 = getVersion();
    if (v1 % 2 == 0) {
      // Already stolen.
      return emptyRes;
...
public:
  StealingMultiQueue(int num_threads, int order=0) : nQ(num_threads), ORDER(order) {
  ...
  #ifdef PAGE_RANK
  void insert(float key, uint32_t value);
  #elif defined(RBP)
  void insert(uint32_t key, J value);
  #else
