...
#ifdef PAGE_RANK
#pragma message ("PAGE_RANK IS DEFINED - Using float")
typedef float slkey_t;
#else
#pragma message ("PAGE_RANK IS NOT DEFINED - Using unsigned long")
typedef unsigned long slkey_t;
#endif
//typedef float slkey_t;

#ifdef RBP
typedef Message* val_t;
#else
typedef unsigned long val_t;
#endif
...
struct alignas(64) sl_node_t
{
  ...
  int toplevel;
  intptr_t deleted;
  struct sl_node_t *next[19];
};

struct alignas(64) sl_intset_t
{
  sl_node_t *head;
};
...
sl_intset_t *sl_set_new(int order=0);
void sl_set_delete(sl_intset_t *set);
int sl_set_size(sl_intset_t *set);

long rand_range(long r); /* declared in test.c */
