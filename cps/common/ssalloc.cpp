...
static __thread long long alloc_next[SSALLOC_NUM_ALLOCATORS] = {0};
...
{
#if !defined(SSALLOC_USE_MALLOC)
  ssalloc_app_mem[0] = (char*)ssalloc_app_mem[0] + size;
#endif
}
...
ssalloc_alloc(unsigned int allocator, size_t size)
{
  ...
  if (ssalloc_free_num[allocator] > 2)
    ...
    {
      ret = (char*)ssalloc_app_mem[allocator] + alloc_next[allocator];
      alloc_next[allocator] += size;
      if (alloc_next[allocator] > SSALLOC_SIZE*4)
      {
        fprintf(stderr, "*** warning: allocator %2d : out of bounds alloc\n", allocator);

        fprintf(stderr, "    Request Size: %zu bytes\n", size);
        fprintf(stderr, "    Current Usage: %ld bytes\n", alloc_next[allocator] - size); // Subtract size to see state before this call
        fprintf(stderr, "    Max Capacity: %ld bytes\n", SSALLOC_SIZE*4);
