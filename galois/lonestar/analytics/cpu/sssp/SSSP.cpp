...
static cll::opt<unsigned int>
    buckets("buckets",
        cll::desc("Number of buckets in hashmap"),
        cll::init(1048576));
static cll::opt<unsigned int>
    batch("batch",
...
const char* const ALGO_NAMES[] = {
    "deltaStepOBIM", "deltaStepPMOD", "MQ", "MQBucket", "serial", "SkipHashPQ", "PIPQ", "SMQ", "Linden", "Spray"};
...
void MQThreadTask(int tid, Graph& graph, MQ &wl, stat_t *stats, std::atomic<uint32_t> *prios, termination_detector_2& detector) {
  ...
  while (true) {
    ...
    //wl.finalize_push() - flush buffer
    // their use of batching introduces more relaxation
  }
  stats->iter = iter;
...
template<typename MQ>
void ThreadTaskPIPQ(int tid, Graph& graph, MQ &wl, stat_t *stats, std::atomic<uint32_t> *prios, termination_detector_2& detector) {
  uint64_t iter = 0UL;
  uint64_t emptyWork = 0UL;
  uint dist;
  GNode src;
  ...
  if (tid) {
    // insert source
    wl.init_thread(tid); // tid == 0 already init to insert the source
  }
...
template<typename MQ>
void ThreadTaskSMQ(int tid, Graph& graph, MQ &wl, stat_t *stats, std::atomic<uint32_t> *prios, termination_detector_2& detector) {
  uint64_t iter = 0UL;
  uint64_t emptyWork = 0UL;
  uint dist;
  GNode src;
...
void STMThreadTask(Graph& graph, SkipHashPQ &wl, stat_t *stats, std::atomic<uint32_t> *prios, termination_detector_2& detector, int tid) {
  uint64_t iter = 0UL;
  uint64_t emptyWork = 0UL;
  uint dist;
  GNode src;
...
template<typename SkipHashPQ, typename descriptor>
void ThreadTaskSTM_Strict(Graph& graph, SkipHashPQ &wl, stat_t *stats, std::atomic<uint32_t> *prios, termination_detector_2& detector, int tid) {
  uint64_t iter = 0UL;
  uint64_t emptyWork = 0UL;
  uint dist;
  GNode src;
...
template<typename SkipHashPQ, typename descriptor>
void STMThreadTaskBatch_1(Graph& graph, SkipHashPQ &wl, stat_t *stats, std::atomic<uint32_t> *prios, termination_detector_2& detector, int tid) {
  uint64_t iter = 0UL;
  uint64_t emptyWork = 0UL;
  uint dist;
  GNode src;
  ...
  while (true) {
    ...
    for (auto e : edgeRange) {
      GNode dst   = graph.getEdgeDst(e);
      newDist = srcD + graph.getEdgeData(e);
#ifdef PERF
      uint32_t oldDist = getPrioData(&prios[dst]);
#else
      uint32_t oldDist = prios[dst].load(std::memory_order_relaxed);
...
template<typename SkipHashPQ, typename descriptor>
void STMThreadTaskBatch_1_ordered(Graph& graph, SkipHashPQ &wl, stat_t *stats, std::atomic<uint32_t> *prios, termination_detector_2& detector, int tid) {
  uint64_t iter = 0UL;
  uint64_t emptyWork = 0UL;
  uint dist;
  GNode src;
...
template<typename SkipHashPQ, typename descriptor>
void spawnTasksSTM(SkipHashPQ& wl, Graph& graph, const GNode& source, int threadNum, std::atomic<uint32_t> *prios, termination_detector_2& detector) {
  // init with source
  auto* me = new descriptor();
  wl.init_thread(me, 0);
  me->op_begin();
  ...
  for (int i = 1; i < threadNum; i++) {
    ...
    if (strict) {
      newThread = new std::thread(
          ThreadTaskSTM_Strict<SkipHashPQ, descriptor>, std::ref(graph), 
          std::ref(wl), &stats[i], std::ref(prios), std::ref(detector), i);
    } else if (order_batch) {
      newThread = new std::thread(
          STMThreadTaskBatch_1_ordered<SkipHashPQ, descriptor>, std::ref(graph), 
          std::ref(wl), &stats[i], std::ref(prios), std::ref(detector), i);
    } else if (batch) {
      newThread = new std::thread(
        STMThreadTaskBatch_1<SkipHashPQ, descriptor>, std::ref(graph), 
        std::ref(wl), &stats[i], std::ref(prios), std::ref(detector), i);
    } else {
      newThread = new std::thread(
        STMThreadTask<SkipHashPQ, descriptor>, std::ref(graph), 
        std::ref(wl), &stats[i], std::ref(prios), std::ref(detector), i);
    } 
  ...
  CPU_SET(0, &cpuset);
  sched_setaffinity(0, sizeof(cpuset), &cpuset);
  if (strict) ThreadTaskSTM_Strict<SkipHashPQ, descriptor>(std::ref(graph), std::ref(wl), &stats[0], std::ref(prios), std::ref(detector), 0);
  else if (order_batch) STMThreadTaskBatch_1_ordered<SkipHashPQ, descriptor>(std::ref(graph), std::ref(wl), &stats[0], std::ref(prios), std::ref(detector), 0);
  else if (batch) STMThreadTaskBatch_1<SkipHashPQ, descriptor>(std::ref(graph), std::ref(wl), &stats[0], std::ref(prios), std::ref(detector), 0);
  else STMThreadTask<SkipHashPQ, descriptor>(graph, wl, &stats[0], prios, std::ref(detector), 0);
...
void spawnTasksPIPQ(PQ_Type& wl, Graph& graph, const GNode& source, int threadNum, std::atomic<uint32_t> *prios, termination_detector_2& detector) {
  ...
  for (int i = 1; i < threadNum; i++) {
    ...
    CPU_SET(coreID, &cpuset);
    std::thread *newThread = new std::thread(
        ThreadTaskPIPQ<PQ_Type>, i, std::ref(graph), 
        std::ref(wl), &stats[i], std::ref(prios), std::ref(detector));
  ...
  ThreadTaskPIPQ<PQ_Type>(0, graph, wl, &stats[0], prios, std::ref(detector));
...
void spawnTasksSpray(PQ_Type& wl, Graph& graph, const GNode& source, int threadNum, std::atomic<uint32_t> *prios, termination_detector_2& detector, thread_data_t *data) {
  ...
  for (int i = 1; i < threadNum; i++) {
    ...
    CPU_SET(coreID, &cpuset);
    std::thread *newThread = new std::thread(
        ThreadTaskSpray<PQ_Type>, i, std::ref(graph), 
  ...
  ThreadTaskSpray<PQ_Type>(0, graph, wl, &stats[0], prios, std::ref(detector), data);
...
void spawnTasksLinden(PQ_Type& wl, Graph& graph, const GNode& source, int threadNum, std::atomic<uint32_t> *prios, termination_detector_2& detector) {
  ...
  for (int i = 1; i < threadNum; i++) {
    ...
    CPU_SET(coreID, &cpuset);
    std::thread *newThread = new std::thread(
        ThreadTaskLinden<PQ_Type>, i, std::ref(graph), 
        std::ref(wl), &stats[i], std::ref(prios), std::ref(detector));
  ...
  ThreadTaskLinden<PQ_Type>(0, graph, wl, &stats[0], prios, std::ref(detector));
...
void spawnTasksSMQ(PQ_Type& wl, Graph& graph, const GNode& source, int threadNum, std::atomic<uint32_t> *prios, termination_detector_2& detector) {
  ...
  for (int i = 1; i < threadNum; i++) {
    ...
    CPU_SET(coreID, &cpuset);
    std::thread *newThread = new std::thread(
        ThreadTaskSMQ<PQ_Type>, i, std::ref(graph), 
        std::ref(wl), &stats[i], std::ref(prios), std::ref(detector));
  ...
  ThreadTaskSMQ<PQ_Type>(0, graph, wl, &stats[0], prios, std::ref(detector));
