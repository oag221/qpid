...
input_dir=bfs-data/inputs/
output_dir=bfs-data/outputs/
...
graphs=(orkut roadnetCA livejournal) # livejournal
chunksize=(1 16 64 128 256)
num_chunks=(1 16 32 64 128 256)
...
for g in "${graphs[@]}"; do
        for q in "${num_chunks[@]}"; do
                for c in "${chunksize[@]}"; do
                        for t in "${threads[@]}"; do
                                filename="${output_dir}/$cnt1.results_bfs_g-${g}.c${c}_queues${q}_threads${t}_SkipHashPQ.log"
                                echo "Results" > $filename

                                cmd="LD_PRELOAD=/usr/lib/x86_64-linux-gnu/libjemalloc.so.2 ${exec_dir}/bfs-cpu -startNode 1 -delta 0 -threads $t -prefetch 1 -stick 1 -chunksize $c -num_chunks $q -algo=SkipHashPQ ${input_dir}/${g}.gr"
...
# LD_PRELOAD=/usr/lib/x86_64-linux-gnu/libjemalloc.so.2 ./build/lonestar/analytics/cpu/bfs/bfs-cpu -startNode 1 -delta 0 -algo=SkipHashPQ -num_chunks 32 -chunksize 64 -threads 96 inputs/orkut.gr